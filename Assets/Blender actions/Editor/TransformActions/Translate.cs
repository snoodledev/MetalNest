...
namespace BlenderActions
{
    public class Translate : BaseTransform
    {
        ...
        public override void Start()
        {
            ...
            OriginalMousePos.y = SceneView.lastActiveSceneView.camera.pixelHeight 
				* EditorGUIUtility.pixelsPerPoint - OriginalMousePos.y;
		...
		private void CalculatePosition()
        {
			...
			Vector2 mousePos = BA.CurrentEvent.mousePosition * EditorGUIUtility.pixelsPerPoint;
			...
			Vector2 objInSP = sceneCam.WorldToScreenPoint(OrigAvgPivot) * EditorGUIUtility.pixelsPerPoint;
			...
			if (BA.TransformSpace == Space.World)
            {
				...
				if (NumericInput.EnteredNumber != 0)//#colreg(lime*3);
				{
					moveTo = LastFrameAvgPivot + new Vector3(NumericInput.EnteredNumber,
						NumericInput.EnteredNumber, NumericInput.EnteredNumber);

					if (BA.TransformLock == Lock.None)
					{
						moveTo.y = LastFrameAvgPivot.y;
						moveTo.z = LastFrameAvgPivot.z;
					}
					...
					else if (BA.TransformLock == Lock.YZ_Plane)
					{
						moveTo.x = LastFrameAvgPivot.x;
						moveTo.z = LastFrameAvgPivot.z;
					}
					...
					else if (BA.TransformLock == Lock.XY_Plane)
					{
						moveTo.x = LastFrameAvgPivot.x;
						moveTo.z = LastFrameAvgPivot.z;
					}
				...
				if (!BA.NumericSnap && BA.VertexSnapON_ThisFrame)
				{
					...
					correctedMousePos.y = sceneCam.pixelHeight 
						* EditorGUIUtility.pixelsPerPoint - correctedMousePos.y;
