...
namespace BlenderActions
{
	public class Utils
	{
		...
		public static Vector3 FindClosestBoxColliderVertex(BoxCollider boxCollider, Camera sceneCam, Vector2 mousePos)
		{
			...
			Vector2 mouseGUIPos = HandleUtility.WorldToGUIPoint(mouseRayWorldSpace.origin) * EditorGUIUtility.pixelsPerPoint;
			...
			{
				// transform GUI-space
				Vector2 vertexGUISpace = HandleUtility.WorldToGUIPoint(vertex) * EditorGUIUtility.pixelsPerPoint;
		...
		public static Rect WorldSpaceBoundsToGUISpaceRect(Bounds bounds)
		{
			...
			{
				HandleUtility.WorldToGUIPoint(new Vector3(cen.x-ext.x, cen.y-ext.y, cen.z-ext.z)) * EditorGUIUtility.pixelsPerPoint,
				HandleUtility.WorldToGUIPoint(new Vector3(cen.x+ext.x, cen.y-ext.y, cen.z-ext.z)) * EditorGUIUtility.pixelsPerPoint,
				HandleUtility.WorldToGUIPoint(new Vector3(cen.x-ext.x, cen.y-ext.y, cen.z+ext.z)) * EditorGUIUtility.pixelsPerPoint,
				HandleUtility.WorldToGUIPoint(new Vector3(cen.x+ext.x, cen.y-ext.y, cen.z+ext.z)) * EditorGUIUtility.pixelsPerPoint,
				HandleUtility.WorldToGUIPoint(new Vector3(cen.x-ext.x, cen.y+ext.y, cen.z-ext.z)) * EditorGUIUtility.pixelsPerPoint,
			...
			ref VertexSnappingResult methodResult)
		{
			...
			if (transform != null && transform.gameObject != null && transform.gameObject.activeInHierarchy && transform.gameObject.activeSelf)
			{
				...
				float distance = Vector2.SqrMagnitude(closestVertexGUISpace 
					- HandleUtility.WorldToGUIPoint(worldPoint) * EditorGUIUtility.pixelsPerPoint);
			...
			bool ignoreZeroesOnX = false, bool ignoreZeroesOnY = false, bool ignoreZeroesOnZ = false, Vector3 pivot = default(Vector3))
		{
			...
			mouseRect.xMin *= EditorGUIUtility.pixelsPerPoint;
			mouseRect.xMax *= EditorGUIUtility.pixelsPerPoint;
			mouseRect.yMin *= EditorGUIUtility.pixelsPerPoint;
			mouseRect.yMax *= EditorGUIUtility.pixelsPerPoint;

			mouseRect.xMin -= sceneCam.pixelWidth * 0.01f * EditorGUIUtility.pixelsPerPoint;
			mouseRect.yMin -= sceneCam.pixelHeight * 0.01f * EditorGUIUtility.pixelsPerPoint;

			mouseRect.xMax += sceneCam.pixelWidth * 0.01f * EditorGUIUtility.pixelsPerPoint;
			mouseRect.yMax += sceneCam.pixelHeight * 0.01f * EditorGUIUtility.pixelsPerPoint;
			...
			for (int i = 0; i < allMeshes.Length; i++)
			{
				...
				if (!checkSuccessful)
				{
						...
						&& skinnedRenderer.sharedMesh != null)
					{
						...
						rect.xMin *= EditorGUIUtility.pixelsPerPoint;
						rect.xMax *= EditorGUIUtility.pixelsPerPoint;
						rect.yMin *= EditorGUIUtility.pixelsPerPoint;
						rect.yMax *= EditorGUIUtility.pixelsPerPoint;
				...
				if (checkSuccessful)
				{
					if (vertices != null)
					{
						...
						while (--n > -1)
						{
							...
							Vector3 worldPoint = currentTransform.TransformPoint(vertex);
							// transform GUI-space
							Vector2 vertexGUISpace = HandleUtility.WorldToGUIPoint(worldPoint) * EditorGUIUtility.pixelsPerPoint;
		...
		public static void DrawDottedLineToTheCenter(Vector3 avgPos)
		{
			...
			Vector3 avgPosGUI = sceneCam.WorldToScreenPoint(avgPos) * EditorGUIUtility.pixelsPerPoint;
			avgPosGUI.z = 3;
			Vector3 avgPosWorld = sceneCam.ScreenToWorldPoint(avgPosGUI);
			//Vector3 avgPosWorld = avgPos;

			// Raw mouse coordinates have y flipped from what unity uses.
			Vector2 mousePos = BlenderActions.Main.CurrentEvent.mousePosition * EditorGUIUtility.pixelsPerPoint;
			mousePos.y = sceneCam.pixelHeight - mousePos.y;

			Vector3 mousePos3DGUI = new Vector3(mousePos.x, mousePos.y, 3);
			...
			if (TransformLock != Lock.None)
			{
				...
				if (!checkSpace || TransformSpace == Space.Self)
				{
					foreach (Transform t in selected)
					{
						...
						if (BlenderActions.Main.Use3DCursor)
							position = originalAvgPivots;

						if (drawX)
						{
							p1 = position + t.TransformDirection(Vector3.right) * lineLength;
							p2 = position + t.TransformDirection(Vector3.left) * lineLength;
						...
						if (drawY)
						{
							p1 = position + t.TransformDirection(Vector3.up) * lineLength;
							p2 = position + t.TransformDirection(Vector3.down) * lineLength;
						...
						if (drawZ)
						{
							p1 = position + t.TransformDirection(Vector3.forward) * lineLength;
							p2 = position + t.TransformDirection(Vector3.back) * lineLength;
